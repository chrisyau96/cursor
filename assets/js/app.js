...
(()=>{
  ...
  function renderHome(){
    ...
    scheduled.forEach(h=>list.appendChild(todayHabitRow(h,now,false)));
  ...
  function todayHabitRow(h,now=hkNow(),flex=false){
    const c=completionOfHabit(h,now); const row=document.createElement('div'); row.className='habit-row '+(c.done?'done':'');
    row.innerHTML=`<div class="habit-icon" style="background:${h.color}22;color:${h.color}">${h.emoji}</div><div class="habit-main"><div class="habit-name"></div><div class="habit-meta"><span>${c.count}/${c.target}</span><span class="mini-dot"></span><span>${frequencyLabel(h)}</span>${c.count?'<span class="mini-dot"></span><span class="entry-stamp">recorded</span>':''}</div><div class="progress-mini"><span style="width:${c.pct}%;background:${h.color}"></span></div></div><button class="check-btn ${c.done?'done':''}" ${c.done?'disabled':''}>${c.done?'✓':'+1'}</button>${c.count?'<button class="icon-btn red" data-reset>↺</button>':''}`;
    row.querySelector('.habit-name').textContent=h.name;
    if(!c.done) row.onclick=async(e)=>{if(e.target.closest('button[data-reset]'))return; await addRecord(h.id,'',todayKey());};
    const reset=row.querySelector('[data-reset]'); if(reset) reset.onclick=async(e)=>{e.stopPropagation(); await resetHabitToday(h.id)};
    return row;
  }
  function renderFlexibleHabits(now=hkNow()){
    ...
    card.style.display='block'; const unfinished=flex.reduce((s,h)=>s+Math.max(0,periodTarget(h)-completionOfHabit(h,now).count),0); $('#flexHabitSummary').textContent=`${unfinished} unfinished target${unfinished===1?'':'s'} this period`; list.innerHTML=''; flex.forEach(h=>list.appendChild(todayHabitRow(h,now,true)));
  ...
  async function resetHabitToday(habitId){const k=todayKey(); state.records=state.records.filter(r=>!(r.date===k&&r.habitId===habitId)); await save(); toast('Habit reset for today')}
  ...
  function openFullLog(){const now=hkNow(); const start=new Date(now); start.setDate(start.getDate()-30); openModal('Full Habit Log',`<div class="log-filter-grid"><div class="field"><label>From</label><input type="date" id="logFrom" value="${dateKey(start)}"></div><div class="field"><label>To</label><input type="date" id="logTo" value="${todayKey()}"></div><button class="btn-inline pink" id="applyLogFilter">Search</button></div><div class="activity-log" id="fullActivityLog" style="margin-top:12px"></div>`); const render=()=>{const from=$('#logFrom').value||trackerStart(), to=$('#logTo').value||todayKey(); const logs=state.records.filter(r=>afterStart(r.date)&&r.date>=from&&r.date<=to).slice().sort((a,b)=>b.at.localeCompare(a.at)); const box=$('#fullActivityLog'); box.innerHTML=logs.length?'':'<div class="empty">No records in this period.</div>'; logs.forEach(r=>box.appendChild(activityItem(r)));}; $('#applyLogFilter').onclick=render; render();}
  function activityItem(r){const h=state.habits.find(x=>x.id===r.habitId)||{}; const div=document.createElement('div'); div.className='activity'; div.innerHTML=`<div class="activity-emoji" style="background:${(h.color||'#4f46e5')}22">${h.emoji||'✓'}</div><div><div class="activity-title"></div><div class="activity-sub">${r.date} · ${new Date(r.at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',timeZone:'Asia/Hong_Kong'})}${r.note?' · '+escapeHtml(r.note):''}</div></div><div class="activity-actions"><button class="btn-inline" data-edit>Edit</button><button class="btn-inline red" data-del>Remove</button></div>`; div.querySelector('.activity-title').textContent=h.name||'Habit'; div.querySelector('[data-edit]').onclick=()=>editRecord(r.id); div.querySelector('[data-del]').onclick=()=>confirm('Remove this record?')&&removeRecord(r.id); return div;}
  function renderHomeJournal(){const k=todayKey(), j=state.journals[k]||{mood:'',energy:5,text:''}; const el=$('#homeJournalForm'); el.innerHTML=`<div class="field"><label>Mood</label><div class="mood-row">${MOODS.map(m=>`<button class="mood ${j.mood===m?'active':''}" data-mood="${m}">${m}</button>`).join('')}</div></div><div class="field"><label>Energy Score</label><div class="energy-panel"><div class="range-value" id="homeEnergyValue">${j.energy}</div><div class="energy-scale"><input type="range" min="0" max="10" value="${j.energy}" id="homeEnergy"><div class="ticks">${Array.from({length:11},(_,i)=>`<span>${i}</span>`).join('')}</div></div></div></div><div class="field journal-area"><label>Reflection</label><textarea id="homeJournalText" placeholder="What went well? What needs adjustment?">${escapeHtml(j.text)}</textarea></div><button class="btn-primary" id="saveJournalHome">Save Journal</button>`;
  ...
  function drawTrend(){
    ...
    const labels=[]; const pctVals=[]; const energy=[];
    for(let i=trendDays-1;i>=0;i--){const d=hkNow();d.setDate(d.getDate()-i);const k=dateKey(d); labels.push(`${d.getMonth()+1}/${d.getDate()}`); pctVals.push(dayPct(d)); energy.push(state.journals[k]?.energy??null);}
    $('#trendNote').textContent='Bars show habit completion. The line shows journal energy score. Blank days are before the start date or have no scheduled habits.';
  ...
  function openDayDetail(k){const d=parseDate(k); const recs=state.records.filter(r=>r.date===k).sort((a,b)=>a.at.localeCompare(b.at)); const j=state.journals[k]; const scheduled=dayScheduledHabits(d); openModal(`Day Detail · ${fmtDate(d)}`,`<div class="small-note">Completion: <strong>${dayPct(d)??0}%</strong>${j?` · Mood ${j.mood||'—'} · Energy ${j.energy}/10`:''}</div><div class="card" style="box-shadow:none;margin-top:12px"><div class="card-head"><h3>Add Record</h3><span class="chip">${scheduled.length} habits</span></div><div class="habit-list" id="dayAddList"></div></div><div class="activity-log" style="margin-top:12px">${recs.length?'':'<div class="empty">No habit records.</div>'}</div>${j?`<div class="journal-item" style="margin-top:12px"><div class="journal-date"><span>Journal</span><span>${j.mood||''} ${j.energy}/10</span></div><div class="journal-text">${escapeHtml(j.text||'')}</div><div class="mini-actions" style="margin-top:10px"><button class="btn-inline" id="editDayJournal">Edit</button><button class="btn-inline red" id="deleteDayJournal">Delete</button></div></div>`:`<div class="mini-actions" style="margin-top:12px"><button class="btn-inline pink" id="addDayJournal">+ Add Journal</button></div>`}`); const addBox=$('#dayAddList'); scheduled.forEach(h=>{const c=completionOfHabit(h,d); const row=document.createElement('div'); row.className='habit-row '+(c.done?'done':''); row.innerHTML=`<div class="habit-icon" style="background:${h.color}22;color:${h.color}">${h.emoji}</div><div class="habit-main"><div class="habit-name">${escapeHtml(h.name)}</div><div class="habit-meta"><span>${c.count}/${c.target}</span><span class="mini-dot"></span><span>${frequencyLabel(h)}</span></div></div><button class="check-btn ${c.done?'done':''}" ${c.done?'disabled':''}>${c.done?'✓':'+1'}</button>`; if(!c.done) row.onclick=async()=>{await addRecord(h.id,'',k); closeModal(); openDayDetail(k)}; addBox.appendChild(row);}); const box=$('.modal .activity-log'); recs.forEach(r=>box.appendChild(activityItem(r))); if(j){$('#editDayJournal').onclick=()=>{closeModal(); openJournalEditor(k)}; $('#deleteDayJournal').onclick=async()=>{if(confirm('Delete journal entry?')){delete state.journals[k]; await save(); closeModal(); toast('Journal deleted')}};} else {const add=$('#addDayJournal'); if(add) add.onclick=()=>{closeModal(); openJournalEditor(k)};}}
  function renderJournals(){const box=$('#journalHistory'); if(!box)return; const keys=Object.keys(state.journals).sort((a,b)=>b.localeCompare(a)); box.innerHTML=''; if(!keys.length){box.innerHTML='<div class="empty">No journal entries yet.</div>'; return;} keys.forEach(k=>{const j=state.journals[k]; const div=document.createElement('div'); div.className='journal-item'; div.innerHTML=`<div class="journal-date"><span>${k}</span><span>${j.mood||''} ${j.energy}/10</span></div><div class="journal-text">${escapeHtml(j.text||'')}</div><div class="mini-actions" style="margin-top:10px"><button class="btn-inline" data-edit>Edit</button><button class="btn-inline red" data-del>Delete</button></div>`; div.querySelector('[data-edit]').onclick=()=>openJournalEditor(k); div.querySelector('[data-del]').onclick=async()=>{if(confirm('Delete this journal?')){delete state.journals[k]; await save(); toast('Journal deleted')}}; box.appendChild(div);});}
  function openJournalEditor(k=todayKey()){const j=state.journals[k]||{mood:'',energy:5,text:''}; openModal('Edit Journal',`<div class="form-grid journal-area"><div class="field"><label>Date</label><input type="date" id="journalDate" value="${k}"></div><div class="field"><label>Mood</label><div class="mood-row">${MOODS.map(m=>`<button class="mood ${j.mood===m?'active':''}" data-mood="${m}">${m}</button>`).join('')}</div></div><div class="field"><label>Energy Score</label><div class="energy-panel"><div class="range-value" id="modalEnergyValue">${j.energy}</div><div class="energy-scale"><input type="range" min="0" max="10" value="${j.energy}" id="modalEnergy"><div class="ticks">${Array.from({length:11},(_,i)=>`<span>${i}</span>`).join('')}</div></div></div></div><div class="field"><label>Reflection</label><textarea id="modalJournalText">${escapeHtml(j.text||'')}</textarea></div><div class="modal-actions"><button class="btn-secondary" data-close>Cancel</button><button class="btn-primary" id="saveJournalModal">Save</button></div></div>`); $$('.mood').forEach(b=>b.onclick=()=>{$$('.mood').forEach(x=>x.classList.remove('active')); b.classList.add('active')}); $('#modalEnergy').oninput=e=>$('#modalEnergyValue').textContent=e.target.value; $('#saveJournalModal').onclick=async()=>{const nk=$('#journalDate').value||k; if(nk!==k) delete state.journals[k]; state.journals[nk]={mood:$('#modalBody .mood.active')?.dataset.mood||'',energy:Number($('#modalEnergy').value),text:$('#modalJournalText').value.trim(),updatedAt:new Date().toISOString()}; await save(); closeModal(); toast('Journal saved')};}
  ...
  function renderLevel(){
    ...
    const xpItems=[]; state.records.filter(r=>afterStart(r.date)).slice().sort((a,b)=>b.at.localeCompare(a.at)).slice(0,15).forEach(r=>{const h=state.habits.find(x=>x.id===r.habitId)||{}; xpItems.push({date:r.date,desc:`${h.emoji||'✓'} ${h.name||'Habit'} completed`,xp:recordXp(r)});}); ledger().slice(0,15).forEach(l=>{if(l.xp) xpItems.push({date:l.date,desc:l.desc,xp:l.xp});}); xpItems.sort((a,b)=>b.date.localeCompare(a.date)); $('#xpHistory').innerHTML=xpItems.length?xpItems.slice(0,20).map(x=>`<div class="ledger-item"><div class="ledger-head"><span>${escapeHtml(x.desc)}</span><span>${x.xp>0?'+':''}${fmtXp(x.xp)} XP</span></div><div class="ledger-sub">${x.date}</div></div>`).join(''):'<div class="empty">No XP activity yet.</div>';
    renderGift();
  }
  function renderGift(){
    ...
    const led=$('#ledgerList'); const l=ledger(); led.innerHTML=l.length?'':'<div class="empty">No reward activity yet.</div>'; l.slice(0,30).forEach(x=>{const div=document.createElement('div'); div.className='ledger-item'; div.innerHTML=`<div class="ledger-head"><span>${x.desc}</span><span>${x.credit?('HK$'+x.credit):(x.type==='gift'||x.type==='redeemGift'?(x.giftIcon||'🎁')+' '+x.gift:'')}</span></div><div class="ledger-sub">${x.date} · XP ${fmtXp(x.xp||0)}</div>`; led.appendChild(div);});
  }
  ...
  function renderSettings(){
    ...
    function drawRewardPanel(tab=rewardActiveTab){
      ...
      if(tab==='credit'){
        p.innerHTML=`<div class="field"><label>Credit Rules</label><div class="small-note">Multiple rules are allowed, but the same completion percentage cannot be duplicated. If a day reaches 100%, all matching lower-percentage rules also apply.</div></div><div id="creditRulesBox"></div><button class="btn-secondary" id="addCreditRule">+ Add Credit Rule</button>`;
        ...
        p.innerHTML=`<div class="field"><label>Gift Rules</label><div class="small-note">Add streak-based gifts. These appear in the Gift page with earned counts and redeem buttons.</div></div><div id="giftRulesBox"></div><button class="btn-secondary" id="addGiftRule">+ Add Gift Rule</button>`;
