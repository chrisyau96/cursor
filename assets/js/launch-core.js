...
(function (root) {
  ...
  Launch.googleErrorHint = function (err) {
    ...
    if (/invalid_client|generaloauthflow|oauth client was not found/i.test(msg)) {
      return 'Google Drive auth failed. Install 63.0.4 from Internal testing. Add Play Console App signing SHA-1 to the Android OAuth client (package com.dincey.habitjournal). Do not paste a client ID on the Play app.';
    }
    ...
    if (/\[16\]|account reauth failed/i.test(msg)) {
      return 'Google Sign-In failed. Install 63.0.4 from Internal testing (Settings footer must say 63.0.4). If it still fails, add Play Console App signing SHA-1 to the Android OAuth client.';
    }
