...
(function (root) {
  ...
  Launch.googleErrorHint = function (err) {
    ...
    if (/invalid_client|generaloauthflow|oauth client was not found/i.test(msg)) {
      return 'Google opened a web OAuth page (invalid_client). Install 63.0.9 from Internal testing and do not paste a client ID on the Play app.';
    }
    ...
    if (/\[16\]|account reauth failed/i.test(msg)) {
      return 'Google Sign-In failed (account reauth). Install 63.0.9. Settings footer must say 63.0.9.';
    }
