...
2. I keep `android/` in the repo. Android Studio → **Generate Signed Bundle** copies the current web app into the AAB (so Settings shows `v62.5`, not a stale `v62`). `npm install && npm run cap:sync` is optional.
