...
assert(Launch.APP_VERSION === '63.0.3', 'app version is 63.0.3');
assert(Launch.googleErrorHint({ message: 'clientId is null or empty' }).includes('Web client ID'), 'missing client id hint');
assert(Launch.googleErrorHint({ message: 'Google Sign-In failed: [16] Account reauth failed' }).includes('63.0.3'), 'error 16 points at 63.0.3');
...
assert(main.includes('StartIntentSenderForResult'), 'Drive auth uses Activity Result API');
...
assert(versionJson.versionName === '63.0.3' && versionJson.versionCode === 16, 'version.json is 63.0.3 / 16');
...
assert(capCfg.includes('/?v=63.0.3'), 'WebView start URL is cache-busted');
...
assert(html.includes("MOMENTUM_APP_VERSION = '63.0.3'"), 'index stamps 63.0.3 in the head');
...
assert(appSrc.includes("||'63.0.3'"), 'in-app footer version is 63.0.3');
...
assert(bundledApp.includes("||'63.0.3'"), 'Android public assets footer is 63.0.3');
...
assert(bundledHtml.includes('63.0.3') && bundledHtml.includes('unregister'), 'bundled index is 63.0.3 and skips native SW');
const bundledCap = readFileSync(path.join(root, '../android/app/src/main/assets/capacitor.config.json'), 'utf8');
assert(bundledCap.includes('/?v=63.0.3'), 'bundled Capacitor config cache-busts the start URL');
...
assert(!plugin.includes('MAX_UI_LAUNCHES'), 'DriveAuth does not launch the account picker twice');
...
assert(plugin.includes('do not show the account picker a second time'), 'silent retry never relaunches the picker');
