...
assert(authorizer.includes('getSignInIntent'), 'DriveAuthorizer uses GoogleSignIn intent, not AuthorizationClient');
assert(authorizer.includes('GoogleAuthUtil.getToken'), 'DriveAuthorizer fetches a Drive access token');
assert(authorizer.includes('https://www.googleapis.com/auth/drive.file'), 'DriveAuthorizer requests drive.file');
assert(!authorizer.includes('Identity.getAuthorizationClient'), 'DriveAuthorizer does not use AuthorizationClient PendingIntent');
assert(!authorizer.includes('onHostResume'), 'DriveAuthorizer does not re-open Google UI on resume');
assert(!authorizer.includes('MAX_UI_LAUNCHES'), 'DriveAuthorizer does not retry the picker three times');
assert(!authorizer.includes('StartIntentSenderForResult'), 'DriveAuthorizer does not launch an IntentSender');
assert(authorizer.includes('launchSignInOnce'), 'DriveAuthorizer opens the account picker once');
...
const nativeAuthorizer = readFileSync(path.join(root, '../native-src/android/java/DriveAuthorizer.java'), 'utf8');
assert(nativeAuthorizer.includes('getSignInIntent'), 'native-src DriveAuthorizer matches the GoogleSignIn path');
