...
assert(Launch.APP_VERSION === '63.0.4', 'app version is 63.0.4');
assert(Launch.googleErrorHint({ message: 'clientId is null or empty' }).includes('Web client ID'), 'missing client id hint');
assert(Launch.googleErrorHint({ message: 'Google Sign-In failed: [16] Account reauth failed' }).includes('63.0.4'), 'error 16 points at 63.0.4');
...
assert(main.includes('StartActivityForResult'), 'Drive consent activity uses Activity Result API');
assert(main.includes('launchDriveConsent'), 'MainActivity launches DriveConsentActivity');
...
assert(versionJson.versionName === '63.0.4' && versionJson.versionCode === 17, 'version.json is 63.0.4 / 17');
...
assert(capCfg.includes('/?v=63.0.4'), 'WebView start URL is cache-busted');
...
assert(html.includes("MOMENTUM_APP_VERSION = '63.0.4'"), 'index stamps 63.0.4 in the head');
...
assert(appSrc.includes("||'63.0.4'"), 'in-app footer version is 63.0.4');
...
assert(bundledApp.includes("||'63.0.4'"), 'Android public assets footer is 63.0.4');
...
assert(bundledHtml.includes('63.0.4') && bundledHtml.includes('unregister'), 'bundled index is 63.0.4 and skips native SW');
const bundledCap = readFileSync(path.join(root, '../android/app/src/main/assets/capacitor.config.json'), 'utf8');
assert(bundledCap.includes('/?v=63.0.4'), 'bundled Capacitor config cache-busts the start URL');
...
assert(plugin.includes('launchDriveConsent'), 'DriveAuth starts DriveConsentActivity');
...
const consent = readFileSync(path.join(root, '../android/app/src/main/java/com/dincey/habitjournal/DriveConsentActivity.java'), 'utf8');
assert(consent.includes('Identity.getAuthorizationClient'), 'DriveConsent uses AuthorizationClient');
assert(consent.includes('https://www.googleapis.com/auth/drive.file'), 'DriveConsent requests drive.file');
assert(consent.includes('launchMode'), 'DriveConsent comment documents standard launchMode');
...
assert(manifest.includes('android:launchMode="standard"'), 'DriveConsentActivity is not singleTask');
